...
class HelpSelect(discord.ui.Select):
    ...
    def _build_options(self) -> list[discord.SelectOption]:
        ...
        for cat, _cmds in sorted(self.cats.items(), key=lambda kv: kv[0].lower()):
            meta = CATEGORY_META.get(cat, FALLBACK_META)
            option_kwargs = {}
            if meta.get("emoji"):
                option_kwargs["emoji"] = meta["emoji"]
            options.append(
...
def build_overview_embed(author: discord.abc.User, cats: dict[str, list[commands.Command]]) -> discord.Embed:
    embed = discord.Embed(
        title="Help Menu",
        description=f"{author.mention}, check the command categories from the dropdown below.",
        color=discord.Color.green(),
    )
    for cat in CATEGORY_META.keys():
        if cat in cats:
            meta = CATEGORY_META[cat]
            prefix = f"{meta['emoji']} | " if meta.get("emoji") else ""
            embed.add_field(name=f"{prefix}{cat} Commands", value=meta["desc"], inline=False)
    return embed
...
def build_category_embed(cat: str, cmds: list[commands.Command]) -> discord.Embed:
    meta = CATEGORY_META.get(cat, FALLBACK_META)
    embed = discord.Embed(
        title=f"{meta['emoji']} | {cat} Commands" if meta.get("emoji") else f"{cat} Commands",
        description=meta["desc"],
        color=discord.Color.green(),
    )
    lines = [f"`!{command.name}` - {command.help or 'No description'}" for command in cmds]
